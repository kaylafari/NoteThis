...
describe("timestamp integrity", () => {
  ...
  it("keeps segment-only timestamps without inventing word alignment", () => {
    const result = normalizeTranscript(
      {
        segments: [{ start: 4, end: 8, text: "A short meeting", speaker: "A" }],
      },
      10,
    );
    expect(result.timing).toBe("segment");
    expect(result.segments[0].words).toEqual([]);
    expect(result.segments[0].start).toBe(4);
  });
  it("labels text-only timing estimated and keeps it inside duration", () => {
    ...
    expect(result.segments[0].words[2]).toEqual({
      text: "three",
      start: 4,
      end: 6,
    });
  ...
  it("rejects malformed timestamps and bounds valid words", () => {
    const result = normalizeTranscript(
      {
        words: [
          { word: "valid", start: -1, end: 12 },
          { word: "wrong", start: 8, end: 2 },
  ...
  it("preserves local Whisper segment words", () => {
        ...
        segments: [
          {
            ...
            text: "Hello",
            words: [{ text: "Hello", start: 0.4, end: 0.8 }],
          },
        ],
      },
      5,
    ...
    expect(result.timing).toBe("word");
    expect(result.segments[0].words[0].start).toBe(0.4);
  });
});
...
describe("catalog and endpoint validation", () => {
  it("contains every current OpenClaw batch speech provider and real browser flows", () => {
    ...
    expect(catalog.stt.map((p) => p.id)).toEqual(
    ...
    expect(catalog.llm.find((p) => p.id === "openai-codex")?.auth).toBe(
      "oauth",
    );
    expect(catalog.llm.find((p) => p.id === "ollama")?.models).toContain(
      "qwen3:0.6b",
    );
  });
  it("accepts local HTTP and remote HTTPS; rejects credential URLs and remote cleartext", () => {
    expect(validateEndpoint("http://127.0.0.1:11434/")).toBe(
      "http://127.0.0.1:11434",
    );
    expect(validateEndpoint("https://models.example.com/v1")).toBe(
...
describe("speech adapter wire formats", () => {
  ...
  it("sends xAI options before the file with native endpoint and no invented model parameter", async () => {
    ...
    const [url, request] = mock.mock.calls[0];
    const keys = [...(request.body as FormData).keys()];
  ...
  it("uses Mistral native timestamp field only when no explicit language is supplied", async () => {
    ...
    expect(
      (mock.mock.calls[0][1].body as FormData).get("timestamp_granularities"),
    ).toBe("segment");
    ...
    expect(
      (mock.mock.calls[1][1].body as FormData).has("timestamp_granularities"),
    ).toBe(false);
  });
  it("uses OpenRouter JSON/base64 audio rather than a multipart OpenAI request", async () => {
    ...
    const [url, request] = mock.mock.calls[0];
    expect(url).toContain("/api/v1/audio/transcriptions");
    expect(JSON.parse(request.body).input_audio).toEqual({
      data: "UklGRg==",
      format: "wav",
    });
  });
  it("redacts provider error bodies and keys", async () => {
    mock.mockResolvedValue(
      new Response("test-secret is rejected, private account info", {
        status: 401,
      }),
    );
    await expect(
      transcribeCloudChunk(audio, 3, settings, "test-secret"),
  ...
  it("uses local Ollama without reading cloud credentials", async () => {
    ...
    const getKey = vi.fn();
    expect(await generateText("system", "question", settings, getKey)).toBe(
      "Local answer.",
    );
    expect(getKey).not.toHaveBeenCalled();
    expect(JSON.parse(mock.mock.calls[0][1].body).think).toBe(false);
  });
});
...
describe("app-owned OAuth lifecycle", () => {
  ...
  it("publishes only login progress, stores tokens, refreshes, then disconnects", async () => {
    ...
    expect(getOAuthState(state.id)?.status).toBe("prompt");
    submitOAuthInput(state.id, "authorization-code");
    await pause();
    expect(getOAuthState(state.id)?.status).toBe("complete");
    expect(JSON.stringify(getOAuthState(state.id))).not.toContain("private");
    expect(await getOAuthConnections()).toContain("test-oauth");
    expect(await getOAuthApiKey("test-oauth")).toBe("private-token");
    saved["test-oauth"].expires = 1;
    expect(await getOAuthApiKey("test-oauth")).toBe("refreshed-token");
    expect(saved["test-oauth"].access).toBe("refreshed-token");
  ...
  it("cancels pending flows and rejects empty codes", async () => {
    ...
    await pause();
    expect(getOAuthState(state.id)?.status).toBe("error");
