# NoteThis setup and developer guide
...
![NoteThis transcript playback](notethis-workspace.png)

## Open an already configured installation
...
Opening the `.app` directly requires your local Ollama service to be running; the `.command` launcher handles the project-local runtime for you. API-key providers can be used without Ollama. On first recording, allow the macOS microphone/system-audio prompts. Use the live source meters to confirm both voices are arriving. See [recording setup](recording.md).

## Fresh setup

These instructions are for someone comfortable using Terminal. The repository is private, so access is required before downloading it. Obtain a copy of the project, then open Terminal in the `AIMeetingNotes` folder. Run the commands below from that folder. Installing prerequisites with `brew` requires Homebrew to be installed first.
...
LLM API adapters come from the installed `@mariozechner/pi-ai` dependency used by OpenClaw. Model choices are queried from the connected account or provider where supported, with explicit labels for bundled fallback suggestions. ChatGPT subscription choices come from its authenticated Codex catalog; a refresh replaces obsolete bundled entries. See [model discovery](model-discovery.md). Its installed OAuth registry exposes browser sign-in for **Anthropic, OpenAI Codex, and GitHub Copilot**. Account access, subscriptions, and provider support still apply. Choose a provider in Settings, click Sign in, open its external browser flow, and complete a callback/code prompt if requested. New credentials belong to NoteThis; existing OpenClaw credentials are not read or modified. If the browser cannot open, the sign-in panel displays the error and offers a copyable link. See [browser sign-in troubleshooting and verification](browser-signin-fix.md).
